...
- **Final dataset:** 231,930 events · 36,440 sessions · 36,405 SSH login attempts · 16,180 commands · 1,322 unique attacker IPs / 84 countries · 20 distinct malware payloads *(corrected 2026-09-19: 83 distinct countries after de-duplicating ip-api labels; 16 uploaded payload hashes — the other 4 were shell-redirect writes)*. Raw events archived to S3 `raw/` (90-day lifecycle) **+ permanent local copies**; SHA-256 manifest at `s3://dram-soc-honeypot-ingest/captured-samples/` (no lifecycle; the zip of the captured files was deleted, all versions, on 2026-09-19 to restore ADR-009); Cowrie TTY replays pulled locally.
