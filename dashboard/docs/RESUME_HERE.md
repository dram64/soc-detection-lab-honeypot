...
- **DONE 2026-09-19:** the **DigitalOcean droplet** (HAProxy ingress + tunnel endpoint) was destroyed; its last write to S3 landed 14:15:21 UTC that day (verified: no new object for ~9 h after, against a 21-min worst-case normal gap). The two fluent-bit IAM users and their access keys were then deleted, and the `edge-shippers-credentials` stack destroyed and removed from the repo. Nothing writes to the ingest bucket any more. **As of 2026-09-19 it is still internet-facing on :22 and its fluent-bit still ships HAProxy connection logs to S3 `raw/haproxy/` every minute** (39,029 objects from 2026-06-18 on; everything older, including all Cowrie data from the run, has expired under the 90-day rule). **These HAProxy-only objects are not a collection run and are not documented or analysed as one**: the droplet kept logging port-22 connections after the Pi (and Cowrie) was gone, so they hold connection metadata with no honeypot sessions. **Decision (2026-09-19): let them expire, no bulk delete.** The `raw/` lifecycle rule expires each object 90 days after it was written (the last one ~2026-12-18 if the droplet is destroyed 2026-09-19), and noncurrent versions 30 days after that (~2027-01-17).
...
|---|---|---|
| (a) ~~Decommission the DigitalOcean droplet~~ **DONE 2026-09-19** | The HAProxy ingress + reverse-tunnel endpoint has nothing to tunnel to now the Pi is gone. Tear down the droplet to stop the ~$4–5/mo spend. Also remove/disable its `soc-fluent-bit` shipping + any DNS pointing at it. | **Recommended next.** Not done yet. Check terraform/edge-shippers state for anything that references the droplet before destroying. |
