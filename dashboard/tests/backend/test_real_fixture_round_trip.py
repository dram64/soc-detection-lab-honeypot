...
def test_real_fixture_carries_no_chat_disclosed_or_user_pc_data(
    ...
    assert "104.174.33.78" not in body, (
        f"{fixture_path.name} still has the maintainer's home IP; re-run sanitize_for_fixture()"
    )
    assert "password_raw" not in body, (
        f"{fixture_path.name} contains password_raw — ADR-005 boundary; re-sanitize before commit"
    )
    assert "fluent_host" not in body and "fluent_source" not in body, (
        f"{fixture_path.name} contains fluent-bit transport metadata; re-sanitize before commit"
    )
...
def test_session_params_message_can_be_a_list() -> None:
    ...
    accept both shapes."""
    raw = json.loads((FIXTURES_DIR / "cowrie_session_params.json").read_text(encoding="utf-8"))
...
def test_log_closed_with_unknown_extras_is_accepted() -> None:
    ...
    raw = json.loads((FIXTURES_DIR / "cowrie_log_closed.json").read_text(encoding="utf-8"))
...
def test_client_kex_with_unknown_extras_is_accepted() -> None:
    ...
    raw = json.loads((FIXTURES_DIR / "cowrie_client_kex.json").read_text(encoding="utf-8"))
